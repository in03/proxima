...
def get_package_last_commit(package_name):
    ...
    try:
        ...
        print("Getting commit ID from package dist info.")
        ...
        print("Couldn't get package dist info, assuming git repo.")
        print("Getting commit ID from git")
        ...
        if output:
            package_last_commit_id = output.decode()
        else:
            print(f"Couldn't get git info!\n" + "Check properly cloned or installed")
            app_exit(1, -1)
